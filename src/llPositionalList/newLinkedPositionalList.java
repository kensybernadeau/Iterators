...
public class newLinkedPositionalList<E> implements PositionalList<E> {
	...
	//new internal reference to iterator maker
	PLIteratorMaker <E> maker;
	
	public newLinkedPositionalList( PLIteratorMaker<E> e) {
		maker =e;
		header = new DNode<>(); 
		trailer = new DNode<>(); 
    ...
    private boolean contains(Position<E> p){
    	Iterable<Position<E>> iter = this.positions();
    	
    	for(Position<E> e: iter){
	...
	public E remove(Position<E> p) throws IllegalArgumentException {
		if(!contains(p)) throw new IllegalArgumentException("invalid position");
		DNode<E> dp = validate(p); 
		E etr = dp.getElement(); 
	...
	public Iterator<E> iterator() {
		//return new PositionListElementsIterator<T>(this);
				return (Iterator<E>) maker.makeIterator(this);
	}
